...
export async function handleResetPassword(formData: FormData) {
  ...
  if (!result.success) {
    ...
    redirect(`/reset-password?error=${encodeURIComponent(errorMessage)}&token=${encodeURIComponent(formData.get("token") as string)}&email=${encodeURIComponent(formData.get("email") as string)}`)
  }
  ...
  redirect("/sign-in?success=password-reset&email=" + encodeURIComponent(email))
}
