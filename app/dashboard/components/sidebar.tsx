...
export function Sidebar() {
  ...
  return (
    <div
      className={`relative flex h-screen flex-col border-r border-border bg-background transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'}`}
    >
      {/* Header */}
      ...
      <div className='flex-1 overflow-y-auto p-2'>
        {/* Item Types */}
        <div className='mb-4'>
          <h3 className={`mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground ${isCollapsed ? 'px-2 text-center' : 'px-2'}`}>
            {isCollapsed ? 'T' : 'TYPES'}
          </h3>
            ...
            {ITEM_TYPES.map((type) => {
              ...
              return (
                <Link
                  key={type.name}
                  href={typePaths[type.name]}
                  className='group flex items-center justify-center gap-3 rounded-lg px-2 py-1.5 text-sm text-muted-foreground transition-colors hover:bg-accent hover:text-foreground'
                >
                  <span
                    className='flex h-5 w-5 shrink-0 items-center justify-center'
                    style={{ color: type.color }}
        ...
        <div className='mb-4'>
          ...
          {!isCollapsed && (
            <div id='collections-section' className='mt-1 space-y-0.5' data-open='true'>
