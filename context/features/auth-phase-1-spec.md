...
1. `auth.config.ts` - Edge-compatible config (providers only, no adapter)
2. `auth.ts` - Full config with Prisma adapter and JWT strategy
3. `app/api/auth/[...nextauth]/route.ts` - Export handlers from auth.ts
4. `proxy.ts` - Route protection with redirect logic
5. `types/next-auth.d.ts` - Extend Session type with user.id
